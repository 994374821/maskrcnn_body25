...
class DataLoader(object):
    def __init__(self, data_root, file_ids, cfg, test_model, is_train=False):
        ...
        self.cfg = cfg
        self.batch_size = cfg.TRAIN.IMS_PER_BATCH
    ...
    def next_batch(self):
        ...
        while True:
            ...
            mask_restop_resize = cv2.resize(mask_res_top.transpose([1,2,0]), (input_size[0], input_size[1]), interpolation=cv2.INTER_LINEAR)
            ...
            data_blob[batch_id, :, :, 20:] = cv2.resize(mask_png_20, (input_size[0], input_size[1]), interpolation=cv2.INTER_LINEAR)
...
def create_model(train, cfg, output_dir):
    ...
    def _single_gpu_build_func(model):
        loss = {"loss": None}
        p = model.Conv("data_stage2", 'conv1_stage2', 40, 20, kernel=3, pad=1, stride=1, no_bias=1, weight_init=(cfg.MRCNN.CONV_INIT, {'std': 0.001}))
        p = model.AffineChannel(p, 'conv1_bn_stage2', dim=20, inplace=True)
        p = model.Relu(p, p)
        human_fc = model.Conv(p, 'conv2_stage2', 20, 20, kernel=1, pad=0, stride=1, weight_init=(cfg.MRCNN.CONV_INIT, {'std': 0.001}))
        if not model.train:
        ...
        if model.train:
            model.net.NCHW2NHWC(human_fc, 'seg_score_NHWC_stage2')
            model.Reshape('seg_score_NHWC_stage2', ['seg_score_reshape_stage2', 'seg_score_old_shape_stage2'], shape=[-1, model.num_classes])
...
def main():
    ...
    train_data = DataLoader(root, "train_id.txt", cfg, test_model, is_train=True)
    # creat mode
    model, weights_file, start_iter, checkpoints = create_model(True, cfg, output_dir)
    # test blob
