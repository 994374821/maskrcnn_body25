...
def add_fpn(model, fpn_level_info):
    ...
    if cfg.FPN.USE_GN:
        ...
        ### add pose to fpn5 lateral
# =============================================================================
        hg_pred_NCHW = model.net.NHWC2NCHW('pose_pred_32', 'hg_pred_NCHW_32')
        model.Relu(hg_pred_NCHW, hg_pred_NCHW)
        hg_pred_NCHW_concat, _ = model.net.Concat([output_blobs[0], hg_pred_NCHW], 
                                                   ['fpn_inner_res5_lateral_pose_concat', 'fpn_inner_res5_2_sum_lateral_info'],
                        axis=1)
        output_blobs[0] = 'fpn_inner_res5_lateral_pose_conv'
        fpn5 = model.Conv(
                   hg_pred_NCHW_concat, output_blobs[0], fpn_dim+26, fpn_dim, kernel=3, stride=1, pad=1,
                   weight_init=('XavierFill', {}),
                   bias_init=const_fill(0.0)
                   )
        model.Relu(fpn5, fpn5)
# =============================================================================
...
# =============================================================================
    ...
    if 'res3' in fpn_lateral or 'res2' in fpn_lateral or 'res4' in fpn_lateral:
         if 'res4' in fpn_lateral:
             hg_pred_NCHW = model.net.NHWC2NCHW('pose_pred_16', 'hg_pred_NCHW_16')
         ...
         hg_pred_NCHW_concat, _ = model.net.Concat([lat, hg_pred_NCHW], 
                       ...
                       axis=1)
         lat = model.Conv(
                  hg_pred_NCHW_concat, fpn_bottom + '_lateral_pose_concat_conv', dim_top+heatmap_dim, dim_top, kernel=3, stride=1, pad=1,
                  weight_init=('XavierFill', {}),
                  bias_init=const_fill(0.0)
                  )
         model.Relu(lat, lat)         
...
def map_mask_rois_to_fpn_levels(rois, k_min, k_max):
    """Determine which FPN level each RoI in a set of RoIs should map to based
    on the heuristic in the FPN paper.
    """
    # Compute level ids
    s = np.sqrt(box_utils.boxes_area(rois))
    s0 = cfg.FPN.MASK_ROI_CANONICAL_SCALE  # default: 224
    lvl0 = cfg.FPN.ROI_CANONICAL_LEVEL  # default: 4
