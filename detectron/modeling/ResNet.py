...
#            model.net.NCHW2NHWC('pose_sum_8', 'pose_sum_NHWC_8')
#            pose_sum = model.net.NHWC2NCHW('pose_sum_NHWC_8', 'pose_sum_NCHW_8')
#            model.StopGradient(pose_sum, pose_sum)
#            pose_sum = 'pose_sum_8'
            pose_line = model.net.PoseLine('pose_line_8', 'pose_line8_w', Cout=512)
            model.StopGradient(pose_line, pose_line)
        if 'res4' in prefix:
#            hg_pred_NCHW_16 = model.net.NHWC2NCHW('pose_pred_16', 'hg_pred_NCHW_16')
#            model.Relu(hg_pred_NCHW_16, hg_pred_NCHW_16)
#            model.net.NCHW2NHWC('pose_sum_16', 'pose_sum_NHWC_16')
#            pose_sum = model.net.NHWC2NCHW('pose_sum_NHWC_16', 'pose_sum_NCHW_16')
#            model.StopGradient(pose_sum, pose_sum)
#            pose_sum = 'pose_sum_16'
            pose_line = model.net.PoseLine('pose_line_16', 'pose_line16_w', Cout=1024)
            model.StopGradient(pose_line, pose_line)
    for i in range(n):
        ...
        if cfg.MODEL.NONLOCAL:
            ...
            if cfg.NONLOCAL.CONV3_NONLOCAL and ('res3' in prefix):
                nonlocal_mod = cfg.NONLOCAL.LAYER_MOD
#                hg_pred_NCHW = hg_pred_NCHW_8
            if cfg.NONLOCAL.CONV4_NONLOCAL and ('res4' in prefix):
#                nonlocal_mod = cfg.NONLOCAL.LAYER_MOD*4-1
                nonlocal_mod = cfg.NONLOCAL.LAYER_MOD
                nonlocal_mod = 100
#                hg_pred_NCHW = hg_pred_NCHW_16
            if i % nonlocal_mod == nonlocal_mod - 1:
# ===========================pose16 concat==================================================
#                 nonlocal_name = 'pose_concat' + '_{}_{}'.format(prefix, i)
# #                blob_in_orig = blob_in
#                 hg_pred_NCHW_concat, _ = model.net.Concat([blob_in, hg_pred_NCHW], [nonlocal_name, nonlocal_name+'_info'],
#                       axis=1)
#                 blob_in = model.Conv(
#                      hg_pred_NCHW_concat, 'pose_conv' + '{}_{}'.format(prefix, i), dim_out+16, dim_out, kernel=3, stride=1, pad=1,
#                      weight_init=('XavierFill', {}),
#                      bias_init=const_fill(0.0)
#                      )
#                 model.Relu(blob_in, blob_in)
# =============================================================================
#                blob_in = model.net.Add([blob_in, pose_sum], nonlocal_name+'_pose_sum')
                blob_in = model.net.Mul([blob_in, pose_line], '{}_{}'.format(prefix, i)+'_pose_mul')
        dim_in = dim_out
