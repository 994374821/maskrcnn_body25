...
#            blob_names += ['rois_hg_fpn'+ str(lvl)]
        blob_names += ['rois_idx_restore_int32']
#        blob_names += ['rois_hg_idx_restore_int32']
        if is_training:
...
def add_fast_rcnn_blobs(blobs, im_info, roidb):
    """Add blobs needed for training Fast R-CNN style models."""
    # Sample training RoIs from each image and append them to the blob lists
    for im_i, entry in enumerate(roidb):
        frcn_blobs = _sample_rois(entry, im_info[im_i], im_i)
...
def _sample_rois(roidb, im_info, batch_idx):
...
#    sample_rois_hg = sampled_boxes * np.array([384.0/im_w, 384.0/im_h, 384.0/im_w, 384.0/im_h], dtype=np.float32)
#    sample_rois_hg = np.hstack((repeated_batch_idx, sample_rois_hg))

    # Base Fast R-CNN blobs
    blob_dict = dict(
        labels_int32=sampled_labels.astype(np.int32, copy=False),
        rois=sampled_rois,
#        rois_hg=sample_rois_hg,
...
#    _distribute_rois_over_fpn_levels('rois_hg')
    if cfg.MODEL.MASK_ON:
