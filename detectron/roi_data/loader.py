...
#import os
#import sys
##sys.path.append(os.path.join(os.path.realpath(__file__), '..', '..', 'LIP_JPPNet'))
...
#        self.pose_pred_model = generate_poseJPPNet_pred_model()
        ...
        with self.coordinator.stop_on_exception():
            while not self.coordinator.should_stop():
                ...
                for key in self.get_output_names():
                    if isinstance(blobs[key], list):
                        logger.info('blob[{}] is list'.format(key))
...
#            blobs, valid = get_minibatch(minibatch_db, self.pose_pred_model)
        return blobs
