...
#            distribute_plus_pose(rois, None, inputs, outputs, self._train)
...
#    else: # test, expect 'im_info', when use hg_rois need 'im_info'
...
#    print('im_info shape:', im_info.shape)
    ...
    hg_rois = rois * 1. / im_scale * np.array([1, 384.0/im_w, 384.0/im_h, 384.0/im_w, 384.0/im_h], dtype=np.float32)
...
#    hg_rois = rois[:, 1:5] * 1. / im_scale * np.array([1, 255.0/im_w, 255.0/im_h, 255.0/im_w, 255.0/im_h], dtype=np.float32)
    ...
    fpn.add_multilevel_roi_blobs(
            blobs, 'rois', blobs['rois'], lvls, lvl_min,
            lvl_max
        )
    fpn.add_multilevel_roi_blobs(
            blobs, 'rois_hg', blobs['rois_hg'], lvls, lvl_min,
            lvl_max
        )
    for i, k in enumerate(output_blob_names):
        blob_utils.py_op_copy_blob(blobs[k], outputs[i])
