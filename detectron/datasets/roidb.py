...
def combined_roidb_for_training(dataset_names, proposal_files):
    ...
    if 'LIP_train'==dataset_name:
        cache_file = os.path.join(cache_path,'lip_gt_roidb.pkl')
    if 'ATR_train'==dataset_name:
        cache_file = os.path.join(cache_path,'ATR_gt_roidb.pkl')
    if 'LIP_val'==dataset_name:
        cache_file = os.path.join(cache_path,'lip_val_gt_roidb.pkl')
#        cache_file = os.path.join(cache_path,'ATR_gt_roidb_crop.pkl')
    if os.path.exists(cache_file):
        with open(cache_file,'rb') as f:
            roidb = cPickle.load(f)
        logger.info('gt roidb load from {}'.format(cache_file))
    else:
...
def extend_with_flipped_entries(roidb, dataset):
    ...
    for entry in roidb:
        ...
        if not cfg.Ignore_left: # when filpped, the right is changed to left 
            dont_copy = ('boxes', 'gt_keypoints', 'flipped', 'gt_classes', 'max_classes', 'gt_overlaps')
