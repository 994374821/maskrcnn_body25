...
from detectron.roi_data.minibatch import _resize_pose_blob
from detectron.roi_data.minibatch import _resize_pose_blob_to13
from detectron.roi_data.minibatch import _resize_poseline_blob

logger = logging.getLogger(__name__)


def im_detect_all(model, im, box_proposals, timers=None, entry=None, pose_model=None):
    if timers is None:
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        scores, boxes, im_scale = im_detect_bbox_aug(model, im, box_proposals, entry=entry)
    else:
        if cfg.STD_NMS:
            ...
            scores, boxes, im_scale = im_detect_bbox(
                model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, boxes=box_proposals, pose_model=pose_model, entry=entry
            )
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None, pose_model=None, entry=None):
    ...
    """
    inputs, im_scale = _get_blobs(im, boxes, target_scale, target_max_size, pose_model, entry)
...
def im_detect_bbox_aug(model, im, box_proposals=None, entry=None):
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        scores_scl, boxes_scl = im_detect_bbox_scale(
            model, im, scale, max_size, box_proposals, entry=entry
        )
        add_preds_t(scores_scl, boxes_scl)

        if cfg.TEST.BBOX_AUG.SCALE_H_FLIP:
            scores_scl_hf, boxes_scl_hf = im_detect_bbox_scale(
                model, im, scale, max_size, box_proposals, hflip=True, entry=entry
            )
    ...
    scores_i, boxes_i, im_scale_i = im_detect_bbox(
        model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, boxes=box_proposals, entry=entry
    )
...
def im_detect_bbox_hflip(
    model, im, target_scale, target_max_size, box_proposals=None, entry=None
):
    ...
    scores_hf, boxes_hf, im_scale = im_detect_bbox(
        model, im_hf, target_scale, target_max_size, boxes=box_proposals_hf, entry=entry
    )
...
def im_detect_bbox_scale(
    model, im, target_scale, target_max_size, box_proposals=None, hflip=False, entry=None
):
    ...
    if hflip:
        scores_scl, boxes_scl, _ = im_detect_bbox_hflip(
            model, im, target_scale, target_max_size, box_proposals=box_proposals, entry=entry
        )
    else:
        scores_scl, boxes_scl, _ = im_detect_bbox(
            model, im, target_scale, target_max_size, boxes=box_proposals, entry=entry
        )
...
def _add_multilevel_rois_for_test(blobs, name):
    ...
    if name=='mask_rois':
        lvls = fpn.map_mask_rois_to_fpn_levels(
                blobs[name][:, 1:5], lvl_min, lvl_max
                )
    else:
...
def _get_blobs(im, rois, target_scale, target_max_size, pose_model=None, entry=None):
    ...
    if entry is not None: # add pose to input
        ...
        if 'ATR' in cfg.TEST.DATASETS[0]:
            blobs['pose_pred_4'], blobs['pose_pred_8'], blobs['pose_pred_16'], blobs['pose_pred_32'] = _resize_pose_blob(pose_blob, 26)
        else:
            blobs['pose_pred_4'], blobs['pose_pred_8'], blobs['pose_pred_16'], blobs['pose_pred_32'] = _resize_pose_blob(pose_blob, 26)
    ...
    if pose_model is not None:
        blobs['pose_pred'] = np.asarray(pose_model.pred_pose_one_img(im), dtype=np.float32)
    
    return blobs, im_scale

def _get_pose_pred(entry):
    """get LIP_JPP pose prediction from .bin file
    """
    num_images = 1
    pred_pose_data = '/home/gaomingda/Downloads/gaomingda/dataset/LIP_JPP_pred_pose/val'
    pose_blob = np.zeros((num_images, 48, 48, 16), dtype=np.float32)
    pred_pose_path = os.path.join(pred_pose_data, entry['id']+'.bin')
    pred_ = np.fromfile(pred_pose_path, dtype=np.float32)
    pred_ = pred_.reshape(48, 48, 16)
    pose_blob[0] = pred_
    return pose_blob


def _get_pose_pred_8_16(entry, data_blob_shape):
    num_images = 1
    pred_pose_data = '/home/gaomingda/Downloads/gaomingda/dataset/LIP_JPP_pred_pose/val'
    pred_pose_path = os.path.join(pred_pose_data, entry['id']+'.bin')
    pred_ = np.fromfile(pred_pose_path, dtype=np.float32)
    ...
    blob_h, blob_w = data_blob_shape[2:4] # NCHW
    pred_ = cv2.resize(pred_, (blob_w, blob_h),
                    interpolation=cv2.INTER_LINEAR)
    pose_blob = np.zeros((num_images, blob_h, blob_w, 16), dtype=np.float32)
    pose_blob[0,:, :, :] = pred_
#    print('pose_blob shape:', pose_blob.shape)
    # return shrink pose_blob to 1./16, 1./8
#    return _resize_pose_blob(pose_blob) 
    # return shrink pose_blob to 1./16, 1./8 and 13 channel
    return _resize_pose_blob_to13(pose_blob)

def _get_pose_pred_16(entry, data_blob_shape):
    num_images = 1
    pred_pose_data = '/home/gaomingda/Downloads/gaomingda/dataset/LIP_JPP_pred_pose/val'
    pred_pose_path = os.path.join(pred_pose_data, entry['id']+'.bin')
    pred_ = np.fromfile(pred_pose_path, dtype=np.float32)
